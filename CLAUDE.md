# my-miyabi-app - Claude Code Context

## プロジェクト概要

**my-miyabi-app** - Miyabiフレームワークで構築された自律型開発プロジェクト
...
このプロジェクトは**miyabi-mcp-bundle v3.7.0**を統合しています。

### 統合機能

- **172+ Tools**: ファイル操作、Git、GitHub、システム監視、Docker等
- **38 Agents**: 自律型タスク実行エージェント群
- **22 Skills**: 高度な開発機能スキル
- **56 Commands**: カスタムコマンド
- **24 Hooks**: イベントフック

### ツール命名規則

- **Miyabi CLI**: `miyabi__init`, `miyabi__status`, `miyabi__agent_run` など
- **Miyabi Bundle**: `miyabi_bundle__git_status`, `miyabi_bundle__file_read` など

### 利用可能なツールカテゴリー

| カテゴリー | ツール数 | 用途 |
|-----------|--------|------|
| Git操作 | 19 | コミット履歴、ブランチ管理等 |
| GitHub統合 | 21 | Issue、PR、プロジェクト管理 |
| ファイル操作 | 30+ | 読み込み、書き込み、検索等 |
| システム監視 | 15+ | CPU、メモリ、ディスク、プロセス |
| Docker/K8s | 20+ | コンテナ管理 |
| データベース | 10+ | クエリ実行、分析 |
| 時間・計算 | 10+ | 変換、計算 |
| その他 | 50+ | ネットワーク、ログ等 |
...
**MCP サーバー登録**: `.claude/mcp.json`
**ソースコード**: `src/mcp/miyabi-bundle/`
**統合ドキュメント**: `src/mcp/miyabi-bundle/INTEGRATION.md`

### ライセンス

miyabi-mcp-bundle は MIT ライセンスで提供されています。
**原著作権**: © 2024 Shunsuke Hayashi
**リポジトリ**: https://github.com/ShunsukeHayashi/miyabi-mcp-bundle
